...
pub const Mailbox = struct {
    ...
    // Called by dist receiver goroutine — delivers message, signals event
    pub fn deliver(self: *Mailbox, msg: Term) !void {
        try self.queue.writeItem(msg);
        self.event.set();
    }

    // Called by actor — blocks (via Io) until message available
    pub fn receive(self: *Mailbox, io: std.Io) !Term {
        while (self.queue.count == 0) {
            try self.event.wait(io); // suspends task, not OS thread
            self.event.reset();
        }
